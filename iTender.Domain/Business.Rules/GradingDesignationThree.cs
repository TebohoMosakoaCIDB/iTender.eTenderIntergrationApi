...
{
    public class GradingDesignationThree : IDesignationRule
    {
        public DesignationGrade GetGrade(List<ContractorModel> contractors)
        {
            ...
            if (contractors.Count >= 3)
            {
                ...
                {
                    //check if the contractor has a grade that matches 2
                    var grade = contractors[i].Grades.Where(g => g.ApprovedGrade == "2" && g.StatusText == "Active").FirstOrDefault();
                ...
                //return the grade
                if (found)
                    return new DesignationGrade() { Grade = "3" };
            }
            //none
